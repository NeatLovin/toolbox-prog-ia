...
- Service serveur : Cloudflare Worker + D1 dans `worker/` (Itération 2). Deux responsabilites : `POST /events` (télémetrie) et `POST /audit` (relais de l'appel Anthropic, cle API jamais exposee au client). Voir la section « Service serveur, télémetrie et consentement » ci-dessous et `worker/README.md`. **Seul `POST /audit` appelle un modele de langage** (classification d'un plan de cours). Le moteur de recommandation (`src/lib/recommendation.js`) est entierement deterministe : aucune couche generative de reformulation de la recommandation n'est implementee (voir la section « Taxonomie des evenements » plus bas pour `reco_generative_used`, et `Choix_stack_technique_PoC_Toolbox.md` section 4.5 pour l'etat de cette decision d'architecture).
- Etat global : Pinia + pinia-plugin-persistedstate (uniquement pour l'audit). Le store src/stores/audit.js persiste le resultat en sessionStorage (jamais localStorage : le resultat derive du plan de cours et ne doit pas survivre a la fermeture de l'onglet sur un poste partage) sous la cle `audit_v1`. Tout acces au stockage passe par `src/lib/safeStorage.js` ou un `try/catch` : un navigateur qui bloque les donnees de site leve une exception des la lecture de `window.localStorage`. Les composables src/composables/ ne gerent pas d'etat global.
...
4. **Arbre de decision** (/arbre) : wizard zone → concept → contexte → objectif cognitif (facultatif, avec hint sous chaque bouton). Pas de question semestre ni de question fonction (Formative par defaut). Résultat en DisclosureCard sur trois niveaux : principe IA de la zone + action immédiate ("À faire maintenant", patron ou premier outil) + badge de provenance + pills des premiers outils, cliquables vers ToolDetailModal (niveau 1), « Patron et outils » (PatronBlock + liste complète des ToolCard, cliquables vers ToolDetailModal) (niveau 2), « Justification et sources » (niveau 3). Mode "toute la zone" disponible. **URL sauvegardable** : quand le résultat est affiché, les paramètres zone/concept/context/bloom sont encodés en query string (router.replace) ; recharger l'URL ou la partager reproduit exactement le même résultat. "Nouvelle recherche" vide les paramètres. Questionnaire de satisfaction (UsabilitySurvey.vue, 6 items dont les 2 UMUX-Lite en tête) affiché sous les actions du résultat, une fois par parcours (arbre et audit indépendamment, pas une fois par session globale).
...
3. **Classification** : appel au Worker Cloudflare (`POST /audit`, voir section suivante) pour chaque section/chunk. Le client n'envoie que `{ session_id, text }` ; modele, `max_tokens` et prompt systeme sont fixes cote Worker (`worker/src/audit.js`), pas cote client. Sortie JSON stricte : `{ is_programming, relevance_confidence, course_summary, sections: [{ concept_ids[], bloom, confidence }] }`. Les IDs de concept sont valides uniquement parmi les 21 de concepts.json. Un document dont le texte extrait depasse `VITE_AUDIT_MAX_DOCUMENT_CHARS` (defaut 160 000, distinct du plafond par requete `AUDIT_MAX_CHARS` cote Worker) est tronque a cette capacite ; l'ecran de resultat affiche alors une banniere neutre indiquant la proportion couverte, et l'evenement `audit_truncated` est emis (voir `worker/README.md` section « Capacite totale d'un document vs plafond par requete »).
...
**Persistance Pinia** : l'etat de l'audit (phase, sections, validated, swot, recommendations, courseContext, courseSummary, isDemo, truncation) est persiste en sessionStorage via pinia-plugin-persistedstate (cle `audit_v1`, `storage: safeSessionStorage` explicite, sans quoi le plugin lit `window.localStorage` hors de tout `try`). Un rechargement de page dans le meme onglet restaure le resultat ; la fermeture de l'onglet l'efface. Un eventuel residu `audit_v1` en localStorage (anciennes versions) est supprime au demarrage (main.js). "Nouvelle analyse" appelle `audit.reset()` qui execute `$reset()` puis supprime la cle. Les champs transitoires (classifications, error, relevanceConfidence) ne sont PAS persistes.
...
src/
  ...
  App.vue, main.js (Pinia + pinia-plugin-persistedstate cree ici avant le mount ; capture de
                  ?src= avant le routeur via lib/campaign.js ; app.config.errorHandler pour app_error)
worker/
...
**Télémetrie** (`src/lib/telemetry.js`) : `track(event, payload)` n'envoie et ne met en file strictement rien tant que `hasConsent()` est faux. **Exception unique** : `submitSurveyResponse()` (voie dédiée `sendDedicatedEvent()`, événement `survey_submitted`) envoie indépendamment du consentement — cliquer « Envoyer » sur le questionnaire de fin de parcours est un acte de consentement explicite et suffisant pour cette seule réponse. Aucune autre voie n'est dédiée : `survey_shown` et `survey_dismissed` (affichage et abandon du questionnaire) passent par `track()` comme n'importe quel autre événement, donc sous la garde normale du consentement général — une première version les faisait passer par la voie dédiée à tort (un affichage automatique et un refus de répondre ne sont pas des actes de consentement), corrigé après recette. Identifiant de session (`src/lib/session.js`, `sessionStorage`, jamais un identifiant persistant), campagne `?src=` capturee une seule fois au demarrage (`captureCampaignOnce()` de `src/lib/campaign.js`, appele depuis main.js) avant que le routeur a diese ne l'efface (un `router.replace({query})` ailleurs dans l'app ecrase la query entiere) ; ce module est la seule source de verite (`getCampaign()` pour la telemetrie et le bandeau), liste blanche fermee `tb2026` | `selftest` (passages de test, exclus de `analysis.sql`) | `direct` pour tout le reste, gardee en memoire en plus du sessionStorage pour survivre a un stockage bloque. File en memoire, flush par lots (10s ou 20 evenements) via `fetch`, flush de fin de session sur `visibilitychange -> hidden` via `sendBeacon`. Mode developpement (`import.meta.env.DEV` ou `VITE_API_BASE` absent) : `console.debug` au lieu du reseau — donc `npm run dev` seul ne peut jamais peupler D1, il faut un build "production" (`vite build`, meme avec `--mode development` pour charger `.env.development`) pour tester la télémetrie en reseau, voir `worker/README.md`.
